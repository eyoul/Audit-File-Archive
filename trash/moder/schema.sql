DROP TABLE IF EXISTS unit_audit_File;
DROP TABLE IF EXISTS department_audit_File;
DROP TABLE IF EXISTS division_audit_File;
...
CREATE TABLE audit_File (
    ...
    FOREIGN KEY (department_id) REFERENCES department (id),
    FOREIGN KEY (unit_id) REFERENCES unit (id),
    FOREIGN KEY (audit_report_id) REFERENCES audit_Report (id),
    FOREIGN KEY (file_type_id) REFERENCES audit_File_Type (id)
);

CREATE TABLE division_audit_File (
    audit_file_id INTEGER NOT NULL,
    division_id INTEGER NOT NULL,
    PRIMARY KEY (audit_file_id, division_id),
    FOREIGN KEY (audit_file_id) REFERENCES audit_File (id),
    FOREIGN KEY (division_id) REFERENCES division (id)
);

CREATE TABLE department_audit_File (
    audit_file_id INTEGER NOT NULL,
    department_id INTEGER NOT NULL,
    PRIMARY KEY (audit_file_id, department_id),
    FOREIGN KEY (audit_file_id) REFERENCES audit_File (id),
    FOREIGN KEY (department_id) REFERENCES department (id)
);

CREATE TABLE unit_audit_File (
    audit_file_id INTEGER NOT NULL,
    unit_id INTEGER NOT NULL,
    PRIMARY KEY (audit_file_id, unit_id),
    FOREIGN KEY (audit_file_id) REFERENCES audit_File (id),
    FOREIGN KEY (unit_id) REFERENCES unit (id)
);